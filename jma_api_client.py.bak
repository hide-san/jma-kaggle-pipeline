...
class JMAApiClient:
    ...
    # ------------------------------------------------------------------ #
    # Cherry Blossom Observations                                          #
    # ------------------------------------------------------------------ #

    @retry(stop=stop_after_attempt(config.RETRY_ATTEMPTS),
           wait=wait_fixed(config.RETRY_WAIT_SECONDS))
    def fetch_cherry_blossom_observations(self) -> pd.DataFrame:
        ...
        for entry in entries:
            try:
                ...
                try:
                    data_resp = _get(data_url)
                    data_root = ET.fromstring(data_resp.content)
                    cherry_data = self._parse_cherry_blossom_xml(data_root, data_url)
                    if cherry_data:
    ...
    def _parse_cherry_blossom_xml(self, root: ET.Element, data_url: str) -> dict | None:
    ...
    @retry(stop=stop_after_attempt(config.RETRY_ATTEMPTS),
           wait=wait_fixed(config.RETRY_WAIT_SECONDS))
    def fetch_volcano_status(self) -> pd.DataFrame:
        ...
        for entry in entries:
            try:
                ...
                try:
                    data_resp = _get(data_url)
                    data_root = ET.fromstring(data_resp.content)
                    volcano_data = self._parse_volcano_status_xml(data_root, data_url)
                    if volcano_data:
    ...
    def _parse_volcano_status_xml(self, root: ET.Element, data_url: str) -> dict | None:
    ...
    # ------------------------------------------------------------------ #
    # Sea Warnings and Forecasts                                          #
    # ------------------------------------------------------------------ #

    @retry(stop=stop_after_attempt(config.RETRY_ATTEMPTS),
           wait=wait_fixed(config.RETRY_WAIT_SECONDS))
    def fetch_sea_warnings(self) -> pd.DataFrame:
        ...
        """
        return self._fetch_sea_data('VPCU51', 'sea_warnings')

    @retry(stop=stop_after_attempt(config.RETRY_ATTEMPTS),
           wait=wait_fixed(config.RETRY_WAIT_SECONDS))
    def fetch_sea_forecasts(self) -> pd.DataFrame:
        ...
        """
        return self._fetch_sea_data('VPCY51', 'sea_forecasts')

    def _fetch_sea_data(self, data_type: str, feed_type: str) -> pd.DataFrame:
        ...
        for entry in entries:
            try:
                ...
                try:
                    data_resp = _get(data_url)
                    data_root = ET.fromstring(data_resp.content)
                    sea_data = self._parse_sea_xml(data_root, data_type)
                    if sea_data:
    ...
    def _parse_sea_xml(self, root: ET.Element, data_type: str) -> dict | None:
